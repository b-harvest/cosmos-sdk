...
func TestFullAppSimulation(t *testing.T) {
	...
	app := NewSimApp(logger, db, nil, true, nil, appOptions, fauxMerkleModeOpt, baseapp.SetChainID(SimAppChainID))
...
func TestAppImportExport(t *testing.T) {
	...
	newApp := NewSimApp(log.NewNopLogger(), newDB, nil, true, nil, appOptions, fauxMerkleModeOpt, baseapp.SetChainID(SimAppChainID))
...
func TestAppStateDeterminism(t *testing.T) {
	...
	for i := 0; i < numSeeds; i++ {
		...
		for j := 0; j < numTimesToRunPerSeed; j++ {
			...
			db := dbm.NewMemDB()
			app := NewSimApp(logger, db, nil, true, nil, appOptions, interBlockCacheOpt(), baseapp.SetChainID(SimAppChainID))
