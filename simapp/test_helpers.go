...
func setup(withGenesis bool, invCheckPeriod uint) (*SimApp, GenesisState) {
	...
	app := NewSimApp(log.NewNopLogger(), db, nil, true, nil, appOptions)
	if withGenesis {
...
func NewSimappWithCustomOptions(t *testing.T, isCheckTx bool, options SetupOptions) *SimApp {
	...
	app := NewSimApp(options.Logger, options.DB, nil, true, nil, options.AppOpts)
...
func NewTestNetworkFixture() network.TestFixture {
	...
	app := NewSimApp(log.NewNopLogger(), dbm.NewMemDB(), nil, true, nil, simtestutil.NewAppOptionsWithFlagHome(dir))

	appCtr := func(val network.ValidatorI) servertypes.Application {
		return NewSimApp(
			val.GetCtx().Logger, dbm.NewMemDB(), nil, true, nil,
