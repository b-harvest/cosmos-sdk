...
func TestSimAppExportAndBlockedAddrs(t *testing.T) {
	...
	app2 := NewSimApp(logger.With("instance", "second"), db, nil, true, nil, simtestutil.NewAppOptionsWithFlagHome(t.TempDir()))
...
func TestRunMigrations(t *testing.T) {
	db := dbm.NewMemDB()
	logger := log.NewTestLogger(t)
	app := NewSimApp(logger.With("instance", "simapp"), db, nil, true, nil, simtestutil.NewAppOptionsWithFlagHome(t.TempDir()))
...
func TestInitGenesisOnMigration(t *testing.T) {
	db := dbm.NewMemDB()
	app := NewSimApp(log.NewTestLogger(t), db, nil, true, nil, simtestutil.NewAppOptionsWithFlagHome(t.TempDir()))
