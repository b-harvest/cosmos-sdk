...
type deliverTxTask struct {
	...
	VersionStores map[storetypes.StoreKey]*multiversion.VersionIndexedStore
...
func (s *scheduler) collectResponses(tasks []*deliverTxTask) []abci.ResponseFinalizeBlock {
	res := make([]abci.ResponseFinalizeBlock, 0, len(tasks))
...
func (s *scheduler) tryInitMultiVersionStore(ctx sdk.Context) {
	...
	mvs := make(map[storetypes.StoreKey]multiversion.MultiVersionStore)
...
func (s *scheduler) prepareTask(task *deliverTxTask) {
	...
	if len(s.multiVersionStores) > 0 {
		...
		vs := make(map[storetypes.StoreKey]*multiversion.VersionIndexedStore)
		...
		ms := cms.SetKVStores(func(k storetypes.StoreKey, kvs storetypes.KVStore) storetypes.CacheWrap {
			return vs[k]
		})
...
func (s *scheduler) executeTask(task *deliverTxTask) {
	...
	doneCh := make(chan abci.ResponseFinalizeBlock)
