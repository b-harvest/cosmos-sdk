...
func (k BaseSendKeeper) addVirtualCoins(ctx context.Context, addr sdk.AccAddress, amt sdk.Coins) {
	...
	store := sdkCtx.TransientStore(k.tVirtualKey)
	...
	coins = coins.Add(amt...)
	store.Set(key, []byte(coins.String()))
}
...
func (k BaseSendKeeper) subVirtualCoins(ctx context.Context, addr sdk.AccAddress, amt sdk.Coins) error {
	...
	spendable, err := sdk.ParseCoinsNormalized(string(value))
	if err != nil {
		panic(err)
	}
	...
	if balance.IsZero() {
		...
		store.Set(key, []byte(balance.String()))
	}
...
func (k BaseSendKeeper) CreditVirtualAccounts(ctx context.Context) error {
	store := sdk.UnwrapSDKContext(ctx).TransientStore(k.tVirtualKey)
	...
	for ; it.Valid(); it.Next() {
		...
		coins, err := sdk.ParseCoinsNormalized(string(it.Value()))
		if err != nil {
			panic(err)
		}
		sum.Add(coins...)
