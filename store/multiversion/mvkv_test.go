...
func TestVersionIndexedStoreGetters(t *testing.T) {
	...
	parentKVStore := cachekv.NewStore(mem)
	mvs := multiversion.NewMultiVersionStore(parentKVStore)
	// initialize a new VersionIndexedStore
	vis := multiversion.NewVersionIndexedStore(parentKVStore, mvs, 1, 2, make(chan types.Abort))
	...
	vis2 := multiversion.NewVersionIndexedStore(parentKVStore, mvs, 3, 2, make(chan types.Abort))
	...
	abortChannel := make(chan types.Abort)
	...
	require.Equal(t, types.ErrReadEstimate, abort.Err)
...
func TestVersionIndexedStoreBoilerplateFunctions(t *testing.T) {
	...
	require.Panics(t, func() { vis.CacheWrap() })
	require.Panics(t, func() { vis.CacheWrapWithListeners(types.NewKVStoreKey("mock"), nil) })
	require.Panics(t, func() { vis.CacheWrapWithTrace(nil, nil) })
